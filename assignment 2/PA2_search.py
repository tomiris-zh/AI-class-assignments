...
def UCSearch(start, goal, barrier):
    ...
    while len(fringe) > 0:
        ...
        if curNode != goal:
            ...
            for i in get_neighbors(curNode):
                cost_of_neighbor = move_cost(i, barrier)
                if i not in fringe and i not in expandedNodes:
                    fringe.add(i)
                    parent[i] = curNode
                    G[i] = cost_of_neighbor + curGscore
                if cost_of_neighbor + G[curNode] < G[i] and i not in expandedNodes:
                    parent[i] = curNode 
                    G[i] = cost_of_neighbor + curGscore
...
def aStarSearch(start, goal, barrier):
    ...
    while len(fringe) > 0:
        ...
        if curNode != goal:
            ...
            for i in get_neighbors(curNode):
                cost_of_neighbor = move_cost(i, barrier)
                total_cost = cost_of_neighbor + G[curNode]
                if i not in fringe and i not in expandedNodes:
                    fringe.add(i)
                    parent[i] = curNode
                    G[i] = total_cost
                    F[i] = total_cost + heuristic(i, goal)
                if cost_of_neighbor + G[curNode] < G[i] and i not in expandedNodes:
                    parent[i] = curNode
                    G[i] = total_cost
                    F[i] = total_cost + heuristic(i, goal)
